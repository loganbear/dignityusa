...
class CRM_Contact_Form_Search_Custom_ContributionAggregateEmail implements CRM_Contact_Form_Search_Interface {
  ...
  function buildForm(&$form) {
     ...
     */
    $this->setTitle('Find Contributors with Email by Aggregate Totals');
    ...
    /**
     * Adding for tax deduction
     */
    $form->add('checkbox', 'tax_deductible', ts('Tax Deductible Only'));

    /**
     * If you are using the sample template, this array tells the template fields to render
     * for the search form.
     */
    $form->assign('elements', array('min_amount', 'max_amount', 'start_date', 'end_date', 'tax_deductible'));
  }
...
LEFT JOIN civicrm_contribution_type cType ON ( cType.id = contrib.contribution_type_id ),
civicrm_contact AS contact_a
LEFT JOIN civicrm_email cEmail ON ( cEmail.contact_id = contact_a.id AND cEmail.is_primary = 1 )
...
";
  ...
  function where($includeContactIDs = FALSE) {
    ...
    if ($this->_formValues['tax_deductible']) {
      $clauses[] = 'cType.is_deductible = 1';
    }
